...
При указании sealed when (возможно, семантика изменится; рассматривают вариант when!!). Sealed when не будут иметь else (ошибка компиляции), но в байткоде else будет присутствовать и будет генерировать исключение.
Текущее поведение
```kotlin
enum class State {  
    OK, FAILED, CREATED  
...
```
Доступно в 1.5.30. А тут  [полное](https://kotlinlang.org/docs/opt-in-requirements.html#experimental-status-of-the-opt-in-requirements) описание
